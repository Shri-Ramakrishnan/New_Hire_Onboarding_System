...
export default function AdminManage() {
  ...
  const loadSteps = async () => {
    try {
      ...
      console.error('Failed to load steps for admin manage:', err);
    }
  };
  ...
  const handleSubmit = async e=>{
    e.preventDefault();
    if (!formData.title||!formData.description||!formData.assignedTo) { setMessage({text:'Fill all fields', type:'error'}); return; }
    try {
      ...
      setMessage({ text:'Failed create step', type:'error' });
    }
  };

  const handleStepUpdate = async (step, action) => {
    try {
      if (action==='deleted') await deleteStep(step._id || step.id);
      else {
        const updates = action==='completed'?{ completed:true, completedAt:new Date().toISOString() }: step;
        await updateStep(step._id || step.id, updates);
      }
      loadSteps();
      setMessage({ text:action==='deleted'?'Deleted successfully':'Updated successfully', type:'success' });
    } catch(err) {
      console.error(err);
          ...
          <div className="container" style={{paddingTop:'24px'}}>
            ...
            <div className="flex-between card">
              ...
              <div className="flex gap-4">
                <Link href="/admin" className="button-secondary">← Back</Link>
                <button onClick={()=>setShowForm(!showForm)} className="button-primary">{showForm?'Cancel':'➕ Add New Step'}</button>
              </div>
            </div>
                  ...
                  {['title','description','assignedTo'].map(name=>(
                    <div key={name} className="input-group">
                      ...
                      {name!=='assignedTo'
                        ...
                        : <select name={name} value={formData.assignedTo} onChange={handleInputChange}>
            ...
            <div className="card">
              ...
              {steps.length ? steps.sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)).map(step=>(
                <StepCard
                ...
                <div>No steps yet. <button onClick={()=>setShowForm(true)}>Create first one</button></div>
              )}
            </div>
          </div>
        </div>
